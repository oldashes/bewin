...
async function importRows(client, item, rows, stockMeta) {
  ...
  for (const row of rows) {
    ...
    signalRecords.push({
      ...
      score: num(row.score),
      model_score: num(row.modelScore),
      amount_ratio: num(row.amountRatio || row.volumeRatio),
      ...
      best_board_type: text(row.bestBoardType),
      best_board_code: text(row.bestBoardCode),
      best_board_name: text(row.bestBoardName),
      best_board_ret_5: num(row.bestBoardRet5),
      best_board_amount_ratio: num(row.bestBoardAmountRatio),
