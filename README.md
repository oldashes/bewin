...
| 股票日 K | 腾讯行情 + 东方财富 | 收益验证、回测、当前收益 | 优先读 Neon；缺失时先用腾讯前复权日 K 补齐，失败后再由东方财富兜底并回写 |
| 沪深300日 K | 腾讯行情 + 东方财富 | 个股收益对照 | 同样采用双源兜底，用于展示同期指数收益和超额收益 |
...
| Cron | UTC 时间 | 北京时间 | 作用 |
|---|---:|---:|---|
| `/api/cron/ths-sync` | 周一到周五 08:35 | 16:35 | 写入同花顺人气快照 |
| `/api/cron/daily-sync` | 周一到周五 08:45 | 16:45 | 收盘后刷新近期策略股票日 K |
| `/api/cron/ths-feature-sync` | 周一到周五 08:55 | 16:55 | 生成同花顺特征池 |
| `/api/cron/daily-signal-sync` | 周一到周五 09:10 | 17:10 | 生成东方财富每日信号特征池 |
| `/api/cron/daily-signal-fallback` | 周一到周五 12:00 | 20:00 | 兜底补跑当日信号 |
...
| `/api/cron/ths-feature-sync` | 生成同花顺策略特征的定时任务 |
| `/api/cron/daily-signal-sync` | 生成东方财富每日信号的定时任务 |
| `/api/cron/daily-signal-fallback` | 兜底补跑每日信号的定时任务 |
...
注意：THS 特征 `metric=attention` 主要覆盖免费热榜前排；早期发现策略需要的 400–1200 深度人气仍依赖付费 iFind 脚本回补（本仓库不内建完整 iFind cron，需自备 token 后单独跑）。
